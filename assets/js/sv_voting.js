jQuery( document ).ready(function() {
    ...
    jQuery('.email_button').click(function(){
        ...
        if(emailVal == '') {
           ...
           jQuery('.email_error').text("Verplicht veld.").css('color','red');
           return false;
        }
        ...
        if(emailVal.match(mailformat)) {
            ...
            jQuery('.email_message').hide();
            jQuery('.email_error').show();
            jQuery('.email_error').text("Je hebt een ongeldig emailadres ingevoerd.").css('color','red');
            return false;
        }
    });
...
function social_sharing(){
    ...
    if(socialType == 'LinkedIn'){
        window.open('https://linkedin.com/shareArticle?mini=true&url=' + link + '&title= '+ title +'&summary=Ik heb net gestemd op '+ title +', deelnemer aan de Accenture Innovation Awards 2014. Bekijk deze en vele andere innovaties en breng ook jouw stem uit.&', 'twitterwindow', 'height=450, width=550, top='+(jQuery(window).height()/2 - 225) +', left='+jQuery(window).width()/2 +', toolbar=0, location=0, menubar=0, directories=0, scrollbars=0');
    }
    if(socialType == 'Twitter'){
        window.open('https://twitter.com/share?url=http://bit.ly/1snIWER&hashtags=AIA14&text=Ik stem op '+ title +' deelnemer aan de Accenture Innovation Awards 2014. Bekijk alle deelnemers en stem ook&', 'twitterwindow', 'height=450, width=550, top='+(jQuery(window).height()/2 - 225) +', left='+jQuery(window).width()/2 +', toolbar=0, location=0, menubar=0, directories=0, scrollbars=0');
    }
}